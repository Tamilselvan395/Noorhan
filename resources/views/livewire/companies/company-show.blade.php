<div class="space-y-6">
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            @if ($order->status->value !== 'delivered' && $order->status->value !== 'cancelled' && ! $order->invoice_id)
                <button wire:click="generateInvoice" class="px-3 py-2 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-xs font-semibold">Generate Invoice</button>
            @elseif ($order->invoice_id)
                <a href="{{ route('invoices.show', $order->invoice_id) }}" class="px-3 py-2 rounded-lg text-xs font-semibold bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-400">View Invoice</a>
            @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            {{-- 📁 Documents --}}
            @include('documents._panel', ['entity' => $order])
        </div>
    </div>
</div>